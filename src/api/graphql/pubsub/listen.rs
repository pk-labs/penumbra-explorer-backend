    ...
    loop {
        match listener.recv().await {
            Ok(notification) => match notification.payload().parse::<i64>() {
                ...
                Err(e) => {
                    warn!(
                        "Failed to parse block notification payload '{}': {}",
...
pub async fn transactions(pubsub: PubSub, pool: Pool<Postgres>) {
    ...
    let mut listener = match PgListener::connect_with(&pool).await {
        ...
        Err(e) => {
            error!(
                "Failed to create PostgreSQL listener for transactions: {}",
                e
            );
    ...
    if let Err(e) = listener.listen("explorer_tx_count_update").await {
        warn!(
            "Failed to LISTEN to explorer_tx_count_update channel: {}",
            e
        );
    }
    ...
    loop {
        match listener.recv().await {
            Ok(notification) => match notification.channel() {
                "explorer_tx_update" => match notification.payload().parse::<i64>() {
                    Ok(height) => {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to parse transaction notification payload '{}': {}",
                            notification.payload(),
                            e
                        );
                    }
                },
                "explorer_tx_count_update" => match notification.payload().parse::<i64>() {
                    Ok(count) => {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to parse transaction count notification payload '{}': {}",
                            notification.payload(),
                            e
                        );
                    }
            ...
            Err(e) => {
                ...
                if let Err(e) = listener.listen("explorer_tx_count_update").await {
                    warn!(
                        "Failed to re-establish LISTEN on explorer_tx_count_update: {}",
                        e
                    );
                }
...
pub async fn transaction_count(pubsub: PubSub, pool: Pool<Postgres>) {
    ...
    let mut listener = match PgListener::connect_with(&pool).await {
        ...
        Err(e) => {
            error!(
                "Failed to create PostgreSQL listener for transaction count: {}",
                e
            );
